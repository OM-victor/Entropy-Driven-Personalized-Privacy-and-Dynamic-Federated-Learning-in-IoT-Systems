...
if args.store:
    ...
    file = open(
        ...
        f'--user_sample_rate {user_sample_rate} '
...
def get_dataset_config():
    if dataset == 'MNIST':
        ...
        clients_train_loaders, clients_test_loaders, client_data_sizes = get_fashionmnist_clients_datasets(train_dataset, num_clients)
        clients_models = [fashionmnistNet() for _ in range(num_clients)]
        global_model = fashionmnistNet()
    else:
        print('undefined dataset')
        assert False
    
    return clients_train_loaders, clients_test_loaders, client_data_sizes, clients_models, global_model


def clip_update(client_update, clipping_bound):
    if args.no_clip:
        return client_update
    
    norm = torch.sqrt(sum(torch.sum(param ** 2) for param in client_update))
    clip_rate = max(1, norm / clipping_bound)
    return [param / clip_rate for param in client_update]


def add_noise(clipped_update, noise_multiplier, clipping_bound, num_clients):
    ...
    return [clipped_param + noise_param for clipped_param, noise_param in zip(clipped_update, noise)]


def aggregate_updates(noisy_updates, sampled_client_weights):
    aggregated_update = []
    for param_index in range(len(noisy_updates[0])):
        stacked_params = torch.stack([noisy_update[param_index] * sampled_client_weights[idx] 
                                    for idx, noisy_update in enumerate(noisy_updates)])
        aggregated_update.append(torch.sum(stacked_params, dim=0))
...
def main():
    ...
    clients_train_loaders, clients_test_loaders, client_data_sizes, clients_models, global_model = get_dataset_config()
    ...
    for epoch in trange(global_epoch):
        ...
        for idx, (client_model, client_trainloader, client_testloader) in enumerate(zip(sampled_clients_models, sampled_clients_train_loaders, sampled_clients_test_loaders)):
            if not args.store:
                tqdm.write(f'client:{idx+1}/{num_clients}')
        ...
        mean_acc = sum(clients_accuracies) / len(clients_accuracies)
        mean_acc_s.append(mean_acc)
        print(mean_acc_s)
        acc_matrix.append(clients_accuracies)
        
        sampled_client_data_sizes = [client_data_sizes[i] for i in sampled_client_indices]
        total_sampled_size = sum(sampled_client_data_sizes)
        sampled_client_weights = [size / total_sampled_size for size in sampled_client_data_sizes]
        
        clipped_updates = [clip_update(client_update, clipping_bound) for client_update in clients_model_updates]
        noisy_updates = [add_noise(clipped_update, noise_multiplier, clipping_bound, num_clients) for clipped_update in clipped_updates]
        aggregated_update = aggregate_updates(noisy_updates, sampled_client_weights)
    ...
    char_set = ''
    ID = ''.join(random.sample(char_set, 5))
